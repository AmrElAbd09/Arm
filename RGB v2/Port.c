...
void PortInit(const strPortConfig_t* ConfigPtr)
{
	...
	for (pin = 0; pin < TOTAL_PORT_NUMBER_OF_PINS; pin++)
	{
        ...
        {  
					...
					CLEAR_BIT(*(volatile uint32*)((volatile uint8*)Portptr + GPIO_GPIOIM_OFFSET), ConfigPtr->pinConfigType[pin].pinType) ; 
        }
        else
        {
            SET_BIT(*(volatile uint32*)((volatile uint8*)Portptr + GPIO_GPIOIM_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					
            if( ConfigPtr->pinConfigType[pin].Interrupt == Port_IntRisingEdge)
            {
              SET_BIT(*(volatile uint32*)((volatile uint8*)Portptr + GPIO_GPIOIEV_OFFSET), ConfigPtr->pinConfigType[pin].pinType)  ;
							
            }
            else if( ConfigPtr->pinConfigType[pin].Interrupt == Port_IntFallingEdge)
            {
                CLEAR_BIT(*(volatile uint32*)((volatile uint8*)Portptr + GPIO_GPIOIEV_OFFSET), ConfigPtr->pinConfigType[pin].pinType)  ;
            }
            else if( ConfigPtr->pinConfigType[pin].Interrupt == Port_IntBothEdges)
            {
                
							SET_BIT(*(volatile uint32*)((volatile uint8*)Portptr + GPIO_GPIOIBE_OFFSET), ConfigPtr->pinConfigType[pin].pinType)  ;
            }
        }
